...
) -> impl Future<Item = HttpResponse, Error = $name_pascal_case$Error> {
    web::block(move || user_manager_register(user_data.into_inner(), pool)).then(|res| match res {
        Ok(user) => Ok(HttpResponse::Ok().json(&user)),
        Err(err) => match err {
            BlockingError::Error(service_error) => Err(service_error),
            BlockingError::Canceled => Err($name_pascal_case$Error::InternalServerError),
        },
    })
}
...
pub fn user_handler_login(
    ...
    web::block(move || user_manager_login(auth_data.into_inner(), pool)).then(
        move |res: Result<SlimUser, BlockingError<$name_pascal_case$Error>>| match res {
            Ok(user) => {
...
pub fn user_handler_me(logged_user: LoggedUser) -> HttpResponse {
    if logged_user.email == None {
        return HttpResponse::Unauthorized().json($name_pascal_case$Error::Unauthorized);
    }
