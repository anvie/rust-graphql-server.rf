...
impl QueryRoot {
    pub fn users(context: &Context) -> Result<Vec<User>, $name_pascal_case$Error> {
        user_manager_get_all(&context)
    }
    pub fn token(context: &Context) -> Result<Token, $name_pascal_case$Error> {
        user_manager_get_jwt(&context)
    }
    pub fn decode(context: &Context) -> Result<&HumanClaims, $name_pascal_case$Error> {
